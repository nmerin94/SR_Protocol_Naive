...
void shift(int w){
	...
	/*for(i = 0; i < w; i++){
		printf("\nIN SHIFT RECV buff[%d] seq = %d", i, recv_buff[i].buffered_packet.seq_no );
	}*/ 
...
int main(int argc, char *argv[]) {
    ...
    if(choice == 1){
    	...
    	while(1){
    		...
    		printf("\nCOUNT BEFORE RECIEVING: %d",count);
    		...
    		l1:
    		//for(i = 0; i<W ; i++)
    			//printf("\nCheck buffer %d th packet seqno = %d", i, recv_buff[i].buffered_packet.seq_no);
    		shift_count = 0;
    		shift_count = sort(count);
    		
    		for(i = 0; i<W ; i++)
    			printf("\nCheck buffer after sorting %d th packet seqno = %d", i, recv_buff[i].buffered_packet.seq_no);

    		printf("\nSHIFT COUNT = %d",shift_count);
    	...
    	while(1)
    	{
			...
			base = recv_buff[0].buffered_packet.seq_no;
			clear_buffer(W);
